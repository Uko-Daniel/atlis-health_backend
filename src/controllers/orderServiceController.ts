...
export const orderController = {
  ...
  async getOrdersByStatus(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      const orders = await orderService.getOrdersByStatus(status, page, limit);
      return reply.status(200).send(orders);
    } catch (err: any) {
      return reply.status(500).send({ error: err.message });
    }
...
export const serviceController = {
  ...
  async searchServices(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      const services = await serviceService.searchServiceByName(name);
      return reply.status(200).send(services);
    } catch (err: any) {
      return reply.status(500).send({ error: err.message });
    }
  ...
  async updateService(request: FastifyRequest, reply: FastifyReply) {
    try {
      const { id }  = request.params as { id: string };
      const service = await serviceService.updateService(id, request.body as any);
      return reply.status(200).send(service);
    } catch (err: any) {
      const status = err.message.includes('not found') ? 404 : 400;
      return reply.status(status).send({ error: err.message });
